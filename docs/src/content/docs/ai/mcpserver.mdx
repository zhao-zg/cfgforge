...
# 启动MCP服务器（默认端口3457）
node <cfggen>/packages/cli/dist/index.js -datadir . -gen mcpserver
```
...
```bash
# 指定端口
node <cfggen>/packages/cli/dist/index.js -datadir . -gen mcpserver,port=8080

# 启用文件监视和自动重载（5秒延迟）
node <cfggen>/packages/cli/dist/index.js -datadir . -gen mcpserver,watch=5,postrun=reload.bat
```
