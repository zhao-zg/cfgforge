...
# 启动MCP服务器（默认端口3457）
node <cfgforge>/packages/cli/dist/index.js -datadir . -gen mcpserver
```
...
claude mcp add --transport http cfgforge http://localhost:3457/mcp
...
```bash
# 指定端口
node <cfgforge>/packages/cli/dist/index.js -datadir . -gen mcpserver,port=8080

# 启用文件监视和自动重载（5秒延迟）
node <cfgforge>/packages/cli/dist/index.js -datadir . -gen mcpserver,watch=5,postrun=reload.bat
```
