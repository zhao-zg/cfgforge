...
```cmd help
Usage: cfgforge [options] [-tool toolName[,param=value...]] -datadir <dir> [-gen genName[,param=value...]]

   or: cfgforge [-h] (print help)
...
-----generators
    ...
    -gen java        生成java代码
        ...
        noSealed             ,默认为false
        beautifulName        美化由snake_case名派生的标识符：类名/getter转PascalCase（factory_animation_type->FactoryAnimationType），enum常量转SCREAMING_SNAKE_CASE，后缀_Entry/_Detail保留，默认false保持原行为
    ...
    -gen javamapper 生成java mapper类（MySQL运行时加载，DataStoreCompat+PBData推送，强类型POJO，替代Python autoCfgFile.py）
        own=null
        dir=mapper           生成目录
        pkg=com.jedi.gameServer.mapper 生成代码的包名
        child=               分号分隔表名，对这些表生成prepareData钩子子类，已存在则跳过
        encoding=UTF-8       生成代码文件的编码
```
...
```bash
pnpm build   # 编译所有 packages（含 packages/cli）
```

### 运行 CLI

```bash
node packages/cli/dist/index.js  # 或通过项目内脚本调用
```
