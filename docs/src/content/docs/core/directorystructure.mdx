...
- **目录命名**（两种格式，优先使用嵌套格式）：
  - **嵌套格式（优先）**：在模块目录下的 `_子表名` 子目录
    ```
    [模块目录]/_[子表名]/
    ```
    例如：表名为 `skill.buff`，存在 `skill/` 模块目录 → `skill/_buff/`
    - 模块目录名支持中文后缀，如 `skill_技能/`（codeName 仍为 `skill`）
    - 支持多层递归：表名 `a.b.c` → 目录 `a/b/_c/`
  - **根级格式（备选）**：当模块目录不存在时回退
    ```
    _[表全名.replace(., _)]/
    ```
    例如：表名为 `skill.buff` → `_skill_buff/`
...
cfgforge 支持两种数据文件组织方式，根据项目规模选择合适的结构：
...
│       other.cfg
│       signin.csv
│       └───_keytest        ← 嵌套 JSON 目录（表 other.keytest）
│               0.json
│               1,2.json
│
├───task
...
│       taskextraexp.csv
│       task_任务.csv
│       └───_task2          ← 嵌套 JSON 目录（表 task.task2）
│               1.json
│               2.json
│               3.json
│
└───_other_keytest          ← 旧格式（根级），与上面的嵌套格式不能同时存在
        0.json
        1,2.json
```
...
| JSON目录 | 优先 `模块/_子名`，备选 `_[表全名.replace(., _)]`| `skill/_buff/` 或 `_skill_buff/` |
