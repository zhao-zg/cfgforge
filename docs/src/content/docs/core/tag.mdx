...
标签系统是 cfgforge 提供的一种灵活的数据筛选机制，用于在代码生成过程中选择性提取配置数据。通过为数据表、结构体和字段添加标签，可以精确控制哪些数据需要被包含在生成的代码中。
...
上面的配置会为客户端生成包含所有字段的 `attack`、`heal`、`buff`、`run`、`sfx` 实现类。但如果客户端只需要类型标识和资源相关字段，不需要数据，可以这样做：
