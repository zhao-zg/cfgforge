...
```bash
node <cfggen>/packages/cli/dist/index.js -datadir <配置目录> -gen verify[,参数]
```
...
```bash
# 执行外键一致性检查
node <cfggen>/packages/cli/dist/index.js -datadir ../example/config -gen verify

# 额外检查未引用记录,入口记录
node <cfggen>/packages/cli/dist/index.js -datadir ../example/config -gen verify,unreferenced,entry
```
...
```bash
# 在 CI 脚本中添加验证步骤
node <cfggen>/packages/cli/dist/index.js -datadir config -gen verify
if [ $? -ne 0 ]; then
