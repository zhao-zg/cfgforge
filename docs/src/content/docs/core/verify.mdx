...
- **编辑器验证**：在 cfgeditor 图形界面中实时查看检测结果，适合开发过程中使用（详见[编辑器功能介绍 - 未引用记录检测](../../editor/overview/#未引用记录检测)）
...
```bash
node <cfgforge>/packages/cli/dist/index.js -datadir <配置目录> -gen verify[,参数]
```
...
```bash
# 执行外键一致性检查
node <cfgforge>/packages/cli/dist/index.js -datadir ../example/config -gen verify

# 额外检查未引用记录,入口记录
node <cfgforge>/packages/cli/dist/index.js -datadir ../example/config -gen verify,unreferenced,entry
```
...
| `SeqFieldMustBeInt` | seq 只能用于 int 类型字段 | 修改字段类型为 int 或移除 seq |
| `BlockFirstColOverlap` | block 字段首列与外层祖先 block 首列重合（通常是内层 block 排在外层 block 元素的首字段位置），该列无法同时标识内外两层 block 的项标识 | 在内层 block 前放一个非 block 的兄弟字段（如 `name`、`id`），让首列错开 |
...
| `SeqValueNotContinuous` | seq 字段的值不连续（应为 0, 1, 2, ...） | 修正字段值使其构成连续序列 |
...
```bash
# 在 CI 脚本中添加验证步骤
node <cfgforge>/packages/cli/dist/index.js -datadir config -gen verify
if [ $? -ne 0 ]; then
...
- [元数据配置](../othermetadatas/) - 了解 `(root)` 元数据配置
- [外键关联](../key/) - 了解外键的定义和使用
- [配置编辑器功能介绍](../../editor/overview/) - 在图形界面中使用验证功能
