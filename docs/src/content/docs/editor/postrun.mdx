...
```bat
node ../packages/cli/dist/index.js -datadir ../example/config -gen server,watch=1
```
...
```bat
node ../packages/cli/dist/index.js -datadir ../example/config -gen server,watch=1,postrun=upload.bat
```
...
**1. 服务器启动命令**
```bat
node <cfgforge>/packages/cli/dist/index.js -datadir ./game_config -gen server,watch=3,postrun=deploy.bat,port=3456
```
