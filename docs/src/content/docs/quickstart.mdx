...
cfggen 已重写为 TypeScript Monorepo。从源码构建 CLI：
...
```bash
# 从项目根目录执行
node <cfggen路径>/packages/cli/dist/index.js -datadir config -gen verify
node <cfggen路径>/packages/cli/dist/index.js -datadir config -gen bytes,dir:.
```
...
cfggen 支持生成多种语言的类型化访问代码：

```bash
node <cfggen路径>/packages/cli/dist/index.js -datadir config -gen ts,dir:../ts -gen bytes
node <cfggen路径>/packages/cli/dist/index.js -datadir config -gen cs,dir:../cs
node <cfggen路径>/packages/cli/dist/index.js -datadir config -gen go,dir:../go
node <cfggen路径>/packages/cli/dist/index.js -datadir config -gen lua,dir:../lua
```
...
或使用 [GitHub Releases](https://github.com/stallboy/cfggen/releases) 下载预编译的 `cfgeditor.exe`。
...
- `example/cfggen_common.bat` - 生成命令公共脚本