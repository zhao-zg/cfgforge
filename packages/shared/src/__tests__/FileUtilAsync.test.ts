import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import * as fs from 'fs';
import * as path from 'path';
import * as os from 'os';
import {
  moveDirFilesToAnotherDirAsync,
  moveOneFileAsync,
  hasFilesAsync,
  assureFileExistAsync,
} from '../FileUtil';
import { setDefaultFileSystem } from '../CfgFileSystem';
import { NodeFileSystem } from '../NodeFileSystem';

describe('FileUtil async (T12.0b)', () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cfgforge-test-'));
    setDefaultFileSystem(new NodeFileSystem());
  });

  afterEach(() => {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  describe('moveDirFilesToAnotherDirAsync', () => {
    it('moves all files from source to target dir', async () => {
      const from = path.join(tmpDir, 'from');
      const to = path.join(tmpDir, 'to');
      fs.mkdirSync(from);
      fs.writeFileSync(path.join(from, 'a.txt'), 'A');
      fs.writeFileSync(path.join(from, 'b.txt'), 'B');

      await moveDirFilesToAnotherDirAsync(from, to);

      expect(fs.existsSync(path.join(to, 'a.txt'))).toBe(true);
      expect(fs.existsSync(path.join(to, 'b.txt'))).toBe(true);
      expect(fs.readFileSync(path.join(to, 'a.txt'), 'utf8')).toBe('A');
    });

    it('clears target dir before moving', async () => {
      const from = path.join(tmpDir, 'from');
      const to = path.join(tmpDir, 'to');
      fs.mkdirSync(from);
      fs.mkdirSync(to);
      fs.writeFileSync(path.join(to, 'old.txt'), 'old');
      fs.writeFileSync(path.join(from, 'new.txt'), 'new');

      await moveDirFilesToAnotherDirAsync(from, to);

      expect(fs.existsSync(path.join(to, 'old.txt'))).toBe(false);
      expect(fs.existsSync(path.join(to, 'new.txt'))).toBe(true);
    });
  });

  describe('moveOneFileAsync', () => {
    it('moves a single file', async () => {
      const from = path.join(tmpDir, 'a.txt');
      const to = path.join(tmpDir, 'b.txt');
      fs.writeFileSync(from, 'content');

      await moveOneFileAsync(from, to);

      expect(fs.existsSync(from)).toBe(false);
      expect(fs.existsSync(to)).toBe(true);
      expect(fs.readFileSync(to, 'utf8')).toBe('content');
    });

    it('overwrites existing target file', async () => {
      const from = path.join(tmpDir, 'new.txt');
      const to = path.join(tmpDir, 'old.txt');
      fs.writeFileSync(from, 'new');
      fs.writeFileSync(to, 'old');

      await moveOneFileAsync(from, to);

      expect(fs.readFileSync(to, 'utf8')).toBe('new');
    });
  });

  describe('hasFilesAsync', () => {
    it('returns false for non-existent dir', async () => {
      expect(await hasFilesAsync(path.join(tmpDir, 'noexist'))).toBe(false);
    });

    it('returns false for empty dir', async () => {
      const dir = path.join(tmpDir, 'empty');
      fs.mkdirSync(dir);
      expect(await hasFilesAsync(dir)).toBe(false);
    });

    it('returns true for dir with files', async () => {
      const dir = path.join(tmpDir, 'has');
      fs.mkdirSync(dir);
      fs.writeFileSync(path.join(dir, 'file.txt'), 'content');
      expect(await hasFilesAsync(dir)).toBe(true);
    });
  });

  describe('assureFileExistAsync', () => {
    it('resolves when file exists', async () => {
      const filePath = path.join(tmpDir, 'exists.txt');
      fs.writeFileSync(filePath, 'content');
      await expect(assureFileExistAsync(filePath)).resolves.toBeUndefined();
    });

    it('rejects when file does not exist', async () => {
      await expect(assureFileExistAsync(path.join(tmpDir, 'noexist.txt'))).rejects.toThrow();
    });

    it('resolves for null input', async () => {
      await expect(assureFileExistAsync(null as any)).resolves.toBeUndefined();
    });
  });
});